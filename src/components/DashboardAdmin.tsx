...
export default function DashboardAdmin() {
    ...
    {[
        ...
        return (
          <div key={item.id} className="space-y-1">
            <button 
              ...
              className={`w-full flex items-center justify-between p-3.5 rounded-2xl transition-all font-bold group ${isFinanceActive ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
            >
      ...
      return (
        <button 
          ...
          className={`w-full flex items-center gap-3.5 p-3.5 rounded-2xl transition-all font-bold group ${activeTab === item.id ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
        >
          <item.icon size={18} className={activeTab === item.id ? 'text-white' : 'text-slate-400 group-hover:text-emerald-600 transition-colors'} />
          <span className="text-xs tracking-tight">{item.label}</span>
        </button>
      );
    })}
 ...
 return (
  <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row pb-20 md:pb-0 relative font-sans text-slate-900 transition-colors duration-300">
    {/* Sidebar (Desktop) */}
    <aside className="w-72 bg-white border-r border-slate-100 p-6 hidden md:flex flex-col shadow-sm z-30 transition-colors duration-300">
      <div className="flex items-center justify-between gap-3 mb-8">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 overflow-hidden rounded-2xl border-2 border-emerald-600/10 p-0.5 bg-white shadow-sm flex items-center justify-center shrink-0">
            <img 
              src="/logo_ra.jpeg" 
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1 leading-none">Arjawinangun</p>
          </div>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto pr-1 -mr-2 scrolling-touch">
        <NavItems />
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
        ...
        >
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-9 h-9 rounded-full overflow-hidden bg-white border border-slate-200 flex items-center justify-center text-slate-600 font-bold shrink-0">
              {userData?.photoURL ? (
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-slate-400 font-medium truncate">
                {user?.email || 'admin@radarusyifa.sch.id'}
              </p>
            </div>
          </div>
        ...
        <button 
          ...
          className="w-full flex items-center gap-2.5 p-2.5 rounded-xl text-rose-500 hover:bg-rose-50 font-bold transition-all text-xs group"
        >
          <LogOut size={14} className="group-hover:-translate-x-1 transition-transform" />
          <span className="text-xs">Keluar Sesi</span>
        </button>
      </div>
    </aside>
 ...
 {[
 { id: 'overview', label: 'Beranda', icon: BarChart },
 ...
 className={`flex flex-col items-center justify-center gap-1 flex-1 transition-all py-1 ${isSidebarOpen ? 'text-emerald-600' : 'text-slate-400'}`}
 >
 <div className={`p-1.5 rounded-xl transition-all ${isSidebarOpen ? 'bg-emerald-50 text-emerald-600 scale-110 shadow-sm' : 'text-slate-400'}`}>
 <Menu size={20} />
 </div>
 ...
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 overflow-hidden rounded-xl border-2 border-emerald-600/10 p-0.5 bg-white shadow-sm flex items-center justify-center">
 <img 
 src="/logo_ra.jpeg" 
 alt="Logo Resmi" 
 className="w-full h-full object-contain" 
 ...
 <div>
 <h2 className="font-display font-black text-slate-800 text-sm">Portal Admin</h2>
 <p className="text-[10px] text-emerald-600 font-bold uppercase">RA Darusyifa</p>
 </div>
 </div>
 ...
 <span>Keluar Sesi</span>
  ...
  {/* Main Content */}
  <main className="flex-1 min-h-screen relative overflow-y-auto scrolling-touch flex flex-col bg-slate-50">
    {/* Desktop & Tablet Top Header Bar */}
    <header className="bg-white border-b border-slate-100 px-6 lg:px-10 py-4 hidden md:flex items-center justify-between sticky top-0 z-20 shadow-xs">
      {/* Search Bar */}
      <div className="relative w-full max-w-md">
        ...
        <input 
          ...
          className="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200/80 rounded-2xl text-xs font-semibold text-slate-800 placeholder:text-slate-400 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all"
        />
      </div>
      ...
      <div className="flex items-center gap-6">
        ...
        >
          ...
          {payments.filter(p => p.status === 'pending').length > 0 && (
            <span className="absolute top-1.5 right-1.5 min-w-[16px] h-4 px-1 bg-rose-500 text-white text-[9px] font-black rounded-full flex items-center justify-center">
        ...
        >
          ...
          <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-emerald-500/20 bg-slate-100 flex items-center justify-center text-slate-600 font-bold shrink-0">
            {userData?.photoURL ? (
              <img src={userData.photoURL} alt="Admin" className="w-full h-full object-cover" />
            ) : (
    ...
    {/* Mobile Header */}
    <div className="md:hidden bg-white border-b border-slate-100 p-4 flex items-center justify-between sticky top-0 z-[100] shadow-sm backdrop-blur-md bg-white/90">
      <div className="flex items-center gap-3">
        ...
        <h2 className="font-display font-black text-slate-900 tracking-tight">Portal Admin</h2>
      </div>
      <button 
        onClick={async () => { await auth.signOut(); navigate('/login'); }}
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 lg:p-10 pb-32 md:pb-12 flex-1 w-full">
 {activeTab === 'overview' && (
 ...
 <p className="text-[8px] text-blue-900 font-black uppercase tracking-wider">Admin Portal</p>
 </div>
 ...
 {/* Dashboard Header - Sticky on Desktop */}
 <div className="md:sticky md:top-0 z-20 bg-slate-50/80 backdrop-blur-md py-4 mb-8 -mx-4 px-4 md:-mx-10 md:px-10 lg:-mx-12 lg:px-12 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200/50 transition-all">
