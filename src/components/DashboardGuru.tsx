...
export default function DashboardGuru() {
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-10 lg:p-12 pb-32 md:pb-12">
          {activeTab === 'overview' && (
            <div className="space-y-6 pb-20 md:pb-0 animate-in fade-in duration-500">
              ...
              {/* ============================================================ */}
              {/* 1. HERO TEACHER IDENTITY CARD (Vibrant Blue Rounded Container) */}
              {/* ============================================================ */}
              <div className="bg-[#2563EB] bg-gradient-to-br from-[#1E5EE6] to-[#2563EB] rounded-[32px] p-5 sm:p-7 text-white shadow-xl shadow-blue-600/20 relative overflow-hidden">
                ...
                <div className="flex items-start gap-4 sm:gap-5 relative z-10 pr-12">
                  {/* Teacher Avatar with Camera Button */}
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight leading-tight truncate">
                      {userData?.name || 'Ustadzah / Guru Pengajar'}
                    </h2>

                    {/* Class Info */}
                    <p className="text-xs sm:text-sm font-black text-[#FFDF00] uppercase tracking-wide mt-0.5">
                      {userData?.assignedClass || userData?.kelas ? `WALI KELAS ${userData.assignedClass || userData.kelas}` : 'PENDIDIK & PENGAJAR'}
                    </p>
                ...
                {/* Bottom Row: NIP / Email & Status Kepegawaian (Responsive, clear, no truncation) */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-4 relative z-10 text-white pt-1">
                  ...
                  <div className="flex items-center gap-3 bg-white/10 sm:bg-transparent p-2.5 sm:p-0 rounded-2xl sm:rounded-none border border-white/10 sm:border-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[9px] font-bold text-blue-100 uppercase tracking-wider leading-none">NIP / NUPTK / Akun</p>
                      <p className="text-xs sm:text-sm font-black text-white tracking-wide mt-1 truncate">
                        {userData?.nip || userData?.nuptk || user?.email || '-'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 bg-white/10 sm:bg-transparent p-2.5 sm:p-0 rounded-2xl sm:rounded-none border border-white/10 sm:border-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[9px] font-bold text-blue-100 uppercase tracking-wider leading-none">Status Kepegawaian</p>
                      <p className="text-xs sm:text-sm font-black text-white tracking-wide mt-1 leading-snug break-words">
                        {userData?.jabatan || userData?.role || 'Tenaga Pendidik Aktif'}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <header className="hidden md:flex justify-between items-center bg-white p-6 rounded-[28px] border border-slate-100 shadow-xs">
                <div>
                  <h2 className="text-2xl font-black text-slate-800 leading-tight tracking-tight">Selamat Datang, {userData?.name || 'Ustadzah'}</h2>
                  <p className="text-slate-500 text-xs font-medium mt-1">Kelola perkembangan belajar, hafalan, dan evaluasi siswa secara efisien.</p>
                </div>
                <div className="flex items-center gap-3">
                  <button onClick={() => setActiveTab('announcements')} className="p-3 bg-slate-50 border border-slate-100 rounded-2xl text-slate-500 hover:text-blue-600 transition-all relative">
                    <Bell size={20} />
                    {announcements.filter(a => !a.target || a.target === 'all' || a.target === 'guru').length > 0 && (
                  ...
                  <button 
                    onClick={handleLogout}
                    className="px-4 py-2.5 bg-rose-50 border border-rose-100 text-rose-600 hover:bg-rose-100 rounded-2xl transition-all flex items-center gap-2 font-bold text-xs shadow-xs"
                    title="Keluar Sesi"
                  >
                    <LogOut size={16} />
                    <span>Keluar</span>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
                {[
                  { label: 'Jumlah Siswa', value: students.length, detail: 'Siswa Terdaftar', color: 'bg-indigo-600 bg-gradient-to-br from-purple-500 to-indigo-600', icon: Users },
                  ...
                  { label: 'Hafalan Lulus', value: hafalanProgress.filter(h => h.status === 'Mumtaz (Lulus)').length, detail: 'Materi Tuntas', color: 'bg-orange-500 bg-gradient-to-br from-amber-400 to-orange-500', icon: Star },
                  ...
                  <div key={i} className={`relative overflow-hidden ${stat.color} p-4 sm:p-5 md:p-6 rounded-[28px] sm:rounded-[32px] text-white shadow-xl shadow-black/5 group hover:scale-[1.02] transition-all flex flex-col justify-between h-44 sm:h-48 md:h-44`}>
                    <div className="absolute -right-4 -bottom-4 opacity-25 group-hover:scale-110 transition-transform rotate-12 pointer-events-none">
                      <stat.icon size={90} />
                    </div>
                    <div className="relative z-10">
                      <p className="text-[9.5px] sm:text-[10px] font-black uppercase tracking-[0.15em] opacity-80 mb-1">{stat.label}</p>
                      <h4 className="font-black tracking-tighter leading-none text-2xl sm:text-3xl md:text-4xl">{stat.value}</h4>
                    </div>
                    <div className="relative z-10 inline-flex items-center self-start px-2.5 py-0.5 bg-white/20 backdrop-blur-md rounded-full text-[9px] sm:text-[10px] font-black tracking-wide uppercase" style={{ WebkitBackdropFilter: 'blur(8px)' }}>
                      {stat.detail}
                    </div>
                  </div>
                ))}
              ...
              <div className="bg-white p-6 sm:p-8 rounded-[32px] border border-slate-100 shadow-xs">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-1.5 h-6 bg-blue-600 rounded-full"></div>
                  <h3 className="text-base sm:text-lg font-black text-slate-800 uppercase tracking-tight">Menu Pintasan Guru</h3>
                </div>
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 sm:gap-6">
                  {[
                    { id: 'students', label: 'Daftar Siswa', icon: Users, color: 'bg-purple-500 bg-gradient-to-br from-purple-400 to-purple-500' },
                    { id: 'penilaian-kelas', label: 'Nilai Masal', icon: Edit, color: 'bg-indigo-600 bg-gradient-to-br from-blue-600 to-indigo-700' },
                    { id: 'progress', label: 'Rapot Siswa', icon: GraduationCap, color: 'bg-blue-500 bg-gradient-to-br from-blue-400 to-blue-500' },
                    { id: 'exams', label: 'Jadwal Ujian', icon: FileCheck, color: 'bg-rose-500 bg-gradient-to-br from-rose-400 to-rose-500' },
                    { id: 'hafalan', label: 'Modul Hafalan', icon: Star, color: 'bg-amber-500 bg-gradient-to-br from-amber-400 to-amber-500' },
                    { id: 'attendance', label: 'Absensi Guru', icon: Camera, color: 'bg-emerald-500 bg-gradient-to-br from-emerald-400 to-emerald-500' },
                    { id: 'kaldik', label: 'Kaldik', icon: Calendar, color: 'bg-pink-500 bg-gradient-to-br from-pink-400 to-pink-500' },
                    { id: 'announcements', label: 'Info Sekolah', icon: Megaphone, color: 'bg-sky-500 bg-gradient-to-br from-sky-400 to-sky-500' },
                    { id: 'profile', label: 'Profil Saya', icon: User, color: 'bg-teal-500 bg-gradient-to-br from-teal-400 to-teal-500' },
                  ].map((item, idx) => (
                    <button 
                      key={idx} 
                      onClick={() => setActiveTab(item.id)}
                      className="group flex flex-col items-center gap-2.5 transition-all text-center"
                    >
                      <div className={`w-14 h-14 sm:w-16 sm:h-16 ${item.color} rounded-[22px] sm:rounded-[26px] shadow-md flex items-center justify-center text-white transition-all group-active:scale-95 group-hover:scale-105`}>
                        <item.icon size={24} className="sm:w-7 sm:h-7" />
                      </div>
                      <span className="text-[11px] sm:text-xs font-bold text-slate-700 tracking-tight leading-tight">{item.label}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                <div className="bg-white p-6 sm:p-8 rounded-[32px] border border-slate-100 shadow-xs overflow-hidden">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-base sm:text-lg font-black text-slate-800">Laporan Belajar Terakhir</h3>
                  ...
                  <div className="space-y-3">
                    {progress.slice(0, 4).map((p) => {
                      ...
                      return (
                        <div key={p.id} className="flex items-center justify-between p-3.5 sm:p-4 bg-slate-50/80 rounded-2xl border border-slate-100">
                          <div className="min-w-0 pr-2">
                            <p className="font-bold text-slate-800 truncate text-xs sm:text-sm">{studentName}</p>
                            <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-tight truncate mt-0.5">{p.title}</p>
                          </div>
                          <div className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase shrink-0 ${p.status === 'Lulus' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                            {p.status || 'Aktif'}
                          </div>
                        </div>
                      );
                    })}
                    {progress.length === 0 && <p className="text-center py-6 text-slate-400 italic text-xs">Belum ada laporan belajar.</p>}
                  </div>
                </div>

                <div className="bg-white p-6 sm:p-8 rounded-[32px] border border-slate-100 shadow-xs">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-base sm:text-lg font-black text-slate-800">Antrian Hafalan {userData?.assignedClass || userData?.kelas ? `(${userData.assignedClass || userData.kelas})` : ''}</h3>
                  ...
                  <div className="space-y-3">
                    {hafalanProgress.filter(h => h.isReadyForTest && students.some(st => st.id === h.studentId)).slice(0, 4).map((h) => {
                      ...
                      return (
                        <div key={h.id} className="flex items-center gap-3 p-3.5 sm:p-4 bg-blue-50/50 rounded-2xl border border-blue-100/80">
                          <div className="w-9 h-9 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center shrink-0">
                            <Star size={18} fill="currentColor" />
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="font-bold text-slate-800 truncate text-xs sm:text-sm">{studentName} <span className="text-[11px] text-slate-400 font-normal">{studentClass}</span></p>
                            <p className="text-[10px] text-blue-600 font-bold uppercase tracking-tight truncate mt-0.5">{mat?.judul || 'Materi'}</p>
                          </div>
                          <button onClick={() => { setEvaluateHafalan(h); setShowHafalanModal(true); }} className="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow-xs shadow-blue-200 transition-all shrink-0">
                            <CheckCircle size={16} />
                          </button>
                        </div>
                      );
                    ...
                    {hafalanProgress.filter(h => h.isReadyForTest && students.some(st => st.id === h.studentId)).length === 0 && (
                      <p className="text-center py-6 text-slate-400 italic text-xs">Tidak ada antrian setoran untuk kelas Anda saat ini.</p>
                    )}
                  </div>
                </div>
              </div>
 ...
 <HafalanProgressTab userClass={userData?.assignedClass || userData?.kelas} />
          ...
          {activeTab === 'kaldik' && (
            <div className="space-y-6 animate-in fade-in duration-500 pb-20">
              {/* Kaldik Banner Header */}
              <div className="bg-[#2563EB] bg-gradient-to-br from-[#1E5EE6] to-[#2563EB] rounded-[32px] p-6 sm:p-8 text-white shadow-xl shadow-blue-600/20 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -translate-y-1/3 translate-x-1/3 blur-2xl pointer-events-none" />
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-white/20 border border-white/30 flex items-center justify-center text-yellow-300 shrink-0 shadow-sm backdrop-blur-md">
                      <Calendar size={26} />
                    </div>
                    <div>
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-yellow-400 text-blue-950 rounded-full text-[9px] font-black uppercase tracking-wider mb-1">
                        <span>•</span>
                        <span>AGENDA RESMI</span>
                      </div>
                      <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">Kalender Pendidikan (Kaldik)</h2>
                      <p className="text-xs text-blue-100 font-medium mt-0.5">RA Darusyifa Arjawinangun - Tahun Ajaran 2026/2027</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5 self-start sm:self-auto">
                    <button
                      ...
                      className="px-4 py-2.5 bg-white/20 hover:bg-white/30 text-white rounded-2xl text-xs font-black uppercase tracking-wider transition-all flex items-center gap-2 border border-white/30 shadow-sm"
                    >
              ...
              {/* Kaldik Component Content */}
              <div className="bg-white rounded-[32px] p-4 sm:p-6 border border-slate-100 shadow-xs overflow-hidden">
                <KaldikIframe />
              </div>
            </div>
          )}
 ...
 <div className="p-6 text-center text-gray-400 text-sm italic">Belum ada riwayat absensi.</div>
                   ...
                   {activeTab === 'profile' && (
            <div className="space-y-8 max-w-3xl mx-auto md:mx-0 animate-in fade-in duration-500 pb-20">
              <div className="bg-white rounded-[32px] p-6 sm:p-8 border border-slate-100 shadow-xs">
                <h3 className="text-xl sm:text-2xl font-black text-slate-800 mb-8">Pengaturan Profil Guru</h3>
                
                <form onSubmit={handleUpdateProfile} className="space-y-8">
                  <div className="flex flex-col items-center gap-4 bg-slate-50 p-8 rounded-[32px] border border-slate-100">
                    <div className="relative group cursor-pointer" onClick={() => profileFileInputRef.current?.click()}>
                      <div className="w-32 h-32 sm:w-36 sm:h-36 rounded-full bg-white overflow-hidden border-4 border-white shadow-xl transition-transform hover:scale-105 flex items-center justify-center">
                        {editPhoto ? (
                    ...
                    <div className="text-center">
                      <button 
                        ...
                        className="text-xs font-black text-blue-600 uppercase tracking-widest hover:underline inline-flex items-center gap-1.5"
                      >
                        <Camera size={14} />
                        <span>Ganti Foto Profil</span>
                      </button>
                      <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-wider">RA Darusyifa Arjawinangun • Portal Guru</p>
                    </div>
                  </div>

                  <div className="space-y-4">
                    <div>
                      <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Nama Lengkap & Gelar</label>
                      <input 
                        type="text" 
                        value={editName} 
                        onChange={(e) => setEditName(e.target.value)} 
                        className="w-full p-3.5 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-emerald-500 focus:bg-white transition-all font-bold text-sm text-slate-800" 
                        placeholder="Nama Guru"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Email / Akun Terdaftar</label>
                      <input 
                        type="text" 
                        value={user?.email || ''} 
                        readOnly 
                        className="w-full p-3.5 bg-slate-100/70 border-2 border-slate-100 rounded-2xl text-slate-400 font-bold text-sm cursor-not-allowed" 
                      />
                    </div>
                  </div>
                  ...
                  <button 
                    type="submit" 
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white p-4 rounded-2xl font-black text-sm shadow-lg shadow-emerald-200 transition-all flex items-center justify-center gap-2 active:scale-95"
                  >
                    <CheckCircle size={18} />
                ...
                <div className="mt-10 pt-8 border-t border-slate-100">
                  <h4 className="text-base sm:text-lg font-black text-slate-800 mb-4">Ubah Kata Sandi (Password)</h4>
                  <form onSubmit={handleChangePasswordProfile} className="space-y-4">
                    <div>
                      <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Password Baru</label>
                      <input 
                        type="password" 
                        value={newPassword}
                    ...
                    <div>
                      <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Konfirmasi Password Baru</label>
                      <input 
                        type="password" 
                    ...
                    <button 
                      type="submit" 
                      className="w-full bg-slate-800 hover:bg-slate-900 text-white p-4 rounded-2xl font-black text-sm shadow-md transition-all flex items-center justify-center gap-2 active:scale-95"
                    >
                      <CheckCircle size={18} />
                ...
                {/* Sesi Akun & Tombol Keluar */}
                <div className="mt-10 pt-8 border-t border-slate-100">
                  <div className="p-6 rounded-[28px] bg-rose-50/60 border border-rose-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                      <h4 className="text-base font-black text-rose-900">Sesi Akun Guru</h4>
                      <p className="text-xs text-rose-700 font-medium mt-0.5">Keluar dari sesi portal guru pada perangkat ini.</p>
                    </div>
                    <button 
                      type="button"
                      onClick={handleLogout}
                      className="w-full sm:w-auto px-6 py-3.5 bg-rose-600 hover:bg-rose-700 text-white font-black rounded-2xl transition-all flex items-center justify-center gap-2 text-xs shadow-md shadow-rose-200 active:scale-95 shrink-0"
                    >
                      <LogOut size={16} />
