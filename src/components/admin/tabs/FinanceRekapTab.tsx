...
export default function FinanceRekapTab({
 ...
 >
 <option value="" className="">Semua Kelas</option>
 {schoolClasses.map((c: any) => <option key={c.id} value={c.name} className="">{c.name}</option>)}
 </select>
 </div>
 ...
 <option key={cat.id} value={cat.name} className="">{cat.name}</option>
 ))}
 </select>
 </div>
 ...
 className="w-full text-[11px] md:text-xs p-3 md:p-4 bg-slate-50 border-none rounded-xl md:rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-gray-700 cursor-pointer appearance-none font-sans"
 >
 <option value="semua" className="">Semua Status</option>
 <option value="lunas" className="">Lunas</option>
 <option value="menunggak" className="">Menunggak</option>
 </select>
 </div>
 </div>
 ...
 className="flex-1 md:flex-none px-6 py-3 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center justify-center gap-2 border border-emerald-100 "
 >
