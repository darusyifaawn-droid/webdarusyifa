...
export default function HafalanTab() {
  ...
  // Combined distinct categories list (supports deleting & editing defaults)
    ...
    try {
      if (editingCategory) {
        ...
        if (editingCategory.id) {
          // Update existing firestore category doc
          ...
          // Default or static category being edited
          ...
          // If old name is different, hide the old default
        ...
        // Update all materials in database having this category
        if (oldName.toLowerCase() !== trimmed.toLowerCase()) {
          const matsToUpdate = materials.filter(m => m.kategori?.toLowerCase() === oldName.toLowerCase());
  ...
  const handleDeleteCategory = async (catName: string, catDocId?: string) => {
    const usageCount = materials.filter(m => m.kategori?.toLowerCase() === catName.toLowerCase()).length;
    ...
    try {
      if (catDocId) {
        ...
        await setDoc(doc(db, 'hafalan_categories', catDocId), {
          isDeleted: true,
          updatedAt: serverTimestamp()
        }, { merge: true });
      } else {
        // Hide default category by saving a deleted marker
        await addDoc(collection(db, 'hafalan_categories'), {
          name: catName,
          isDeleted: true,
          isDefault: true,
      ...
      if (editingCategory?.name?.toLowerCase() === catName.toLowerCase()) {
