          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody className="divide-y divide-gray-50/50">
              {filteredLogPayments.length === 0 ? (
                <tr><td colSpan={6} className="px-8 py-20 text-center text-gray-300 font-black text-[10px] uppercase tracking-widest">Tidak ada data riwayat</td></tr>
              ) : (
                filteredLogPayments.map((pay) => {
                  const student = allUsers.find(u => u.id === pay.studentId);
                  const statusColor = pay.status === 'lunas' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 
                    ...
                    <tr key={pay.id} className="hover:bg-slate-50/30 transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex flex-col gap-1">
                          <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">{pay.createdAt ? (pay.createdAt.toDate ? pay.createdAt.toDate().toLocaleString() : new Date(pay.createdAt).toLocaleString()) : '-'}</p>
                          <p className="font-bold text-gray-700 text-sm">{pay.description}</p>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center text-indigo-600 font-black text-[10px] border border-gray-200">
                            {student?.name?.charAt(0) || '?'}
                          </div>
                          <span className="truncate max-w-[150px] font-black text-gray-700">{student?.name || 'Unknown'}</span>
                        </div>
                      </td>
